...
int CINT1e1r_loop(__MD *gctr, CINTEnvVarsR *envs, CINTOpt *opt, double *cache)
{
        int *shls  = envs->shls;
        int *atm = envs->atm;
        int *bas = envs->bas;
...
int CINT1e1r_11_loop(__MD *gctr, CINTEnvVarsR *envs, CINTOpt *opt, double *cache)
{
        int *shls  = envs->shls;
        int *atm = envs->atm;
        int *bas = envs->bas;
        ...
        int nf = envs->nf;
        int n_comp = envs->ncomp_e1 * envs->ncomp_tensor;
        int nc = envs->nf * envs->x_ctr[0] * envs->x_ctr[1];
                 ...
                 double *cache, void (*f_c2s)())
{
        ...
        if (opt != NULL) {
                ...
                has_value = CINTf_1e1r_loop[n](gctr, envs, opt, cache);
        } else {
